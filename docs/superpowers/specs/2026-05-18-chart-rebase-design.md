...
### 8.1 Reading the upstream version list

A Helm chart repository is an HTTP server that publishes a single file `index.yaml` at the root of the repo URL:

```
<repo>/index.yaml
```

So for `charts.json` entry `repo: https://istio-release.storage.googleapis.com/charts`, we `curl` the URL `https://istio-release.storage.googleapis.com/charts/index.yaml`.

This file is generated by `helm repo index` and has a stable, predictable structure. Real example (truncated):
...
entries:
  ...
  - apiVersion: v2
    ...
    name: ambient
    urls:
    - https://istio-release.storage.googleapis.com/charts/samples/ambient-1.30.0-rc.0.tgz
    version: 1.30.0-rc.0
  - apiVersion: v2
    appVersion: 1.30.0-beta.0
    urls:
    - https://istio-release.storage.googleapis.com/charts/samples/ambient-1.30.0-beta.0.tgz
...
Per chart name under `entries:`, there is a list of version blocks. The list items use the YAML compact form where `- ` sits at the same indent column as the parent key (so `ambient:` at column 2 and `- apiVersion: v2` also at column 2). Continuation fields of each version block are at column 4.

We only need two fields per version block: `version` and the first entry of `urls`. Everything else is ignored.

**Targeted reader in `_lib.py` (~40 lines stdlib Python):**

A line-by-line state machine that tracks:
1. Whether we have passed the `entries:` line.
2. Whether the current chart name (2-space-indented `<name>:`) matches the chart we're looking for.
3. When inside a matching chart, whether each `  - ` line opens a new version block; while inside a version block, capture `    version:` and the first item under `    urls:`.

Pseudocode:

```python
def list_versions(index_yaml_text: str, chart_name: str) -> list[tuple[str, str]]:
    """Return [(version, tarball_url), ...] for chart_name from a helm index.yaml."""
    out = []
    in_entries = False
    in_target = False
    cur_version = None
    ...
    for raw in index_yaml_text.splitlines():
        ...
        # New chart name section ("  <name>:")
        if line.startswith("  ") and not line.startswith("   ") and line.endswith(":"):
            # Flush previous version block if any
            if in_target and cur_version is not None:
        ...
        # New version block ("  - apiVersion: ...")
        if line.startswith("  - "):
            if cur_version is not None:
                out.append((cur_version, cur_url or ""))
            cur_version, cur_url = None, None
            expecting_url_list = False
            # The "- apiVersion: v2" line itself is just a marker; fields follow
            continue
        ...
        elif expecting_url_list and line.startswith("    - "):
            if cur_url is None:
                cur_url = line.split("- ", 1)[1].strip()
            # ignore subsequent urls
        elif expecting_url_list and not line.startswith("    "):
            expecting_url_list = False
...
**Assumed input constraints (always true for helm-generated `index.yaml`):**
- 2-space indentation
- No flow style (`{...}`, `[...]`)
- No anchors / aliases
- No multi-line scalars in `version` or `urls`
- `version` and `urls` always present in each version block

**URL resolution:** if the captured URL is absolute (starts with `http://` or `https://`), use as-is. If relative (e.g. `gateway-1.22.0.tgz`), resolve against the repo base URL.

We do not aim to be a general YAML parser. If `index.yaml` ever deviates from helm's format, we fix the reader.
